...
import { getGenreNames } from '../utils/formatter';
...
export const ShowDetail = () => {
  ...
  // Clear data if the user switches to a new show
  if (id !== prevId) {
    setPrevId(id);
    setLoading(true);
    setShow(null);
  ...
  useEffect(() => {
    ...
    fetch(`https://podcast-api.netlify.app/id/${id}`)
      .then((res) => {
        if (!res.ok) throw new Error('Could not load show details.');
        return res.json();
      })
      .then((data) => {
        if (isMounted) {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '24px', marginBottom: '32px', flexWrap: 'wrap' }}>
        <img src={show.image} alt={show.title} style={{ width: '200px', height: '200px', borderRadius: '8px', objectFit: 'cover', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }} />
        ...
        <div style={{ flex: '1', minWidth: '280px' }}>
          ...
          {/* Handle mixed genre data (IDs or strings) */}
          <div style={{ margin: '0 0 16px 0', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {show.genres?.map((genre, index) => {
              const genreName = typeof genre === 'number' ? getGenreNames([genre])[0] : genre;
              return (
                <span key={index} style={{ background: '#e2e8f0', color: '#4a5568', padding: '4px 10px', borderRadius: '16px', fontSize: '0.8rem', fontWeight: '500' }}>
                  {genreName}
                </span>
              );
            })}
      ...
      <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', marginBottom: '20px' }}>
          <label htmlFor="season-select" style={{ fontWeight: '600', color: '#1a202c' }}>Browse Seasons:</label>
          <select 
            id="season-select" 
        ...
        {currentSeasonData ? (
          <div style={{ background: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px solid #f1f5f9' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px', borderBottom: '2px solid #e2e8f0', paddingBottom: '12px' }}>
              <img src={currentSeasonData.image || show.image} alt={currentSeasonData.title} style={{ width: '64px', height: '64px', borderRadius: '6px', objectFit: 'cover' }} />
              <h2 style={{ margin: '0', fontSize: '1.4rem', color: '#1a202c' }}>{currentSeasonData.title}</h2>
            </div>

            {currentSeasonData.episodes?.map((episode, idx) => (
              <div key={episode.episode || idx} style={{ padding: '16px 0', borderBottom: idx === currentSeasonData.episodes.length - 1 ? 'none' : '1px solid #e2e8f0' }}>
                <h4 style={{ margin: '0 0 6px 0', fontSize: '1.05rem', color: '#2d3748' }}>{episode.episode || (idx + 1)}. {episode.title}</h4>
                <p style={{ color: '#718096', fontSize: '0.9rem', margin: '0', lineHeight: '1.5' }}>
                  {episode.description ? (episode.description.length > 150 ? `${episode.description.substring(0, 150)}...` : episode.description) : 'No description provided.'}
                </p>
              </div>
            ))}
          </div>
